# Minecraft Latent Planner

## Purpose

Implement a Model Predictive Control (MPC) planner that operates in the latent space of a trained LeWorldModel, using CEM optimization (with optional warm-starting and gradient refinement) to produce action sequences that drive the agent from a current state to a goal state or subgoal target.
