...
Purpose:
- Provide a passive, real-time OpenCV window that displays the agent's POV
  frame as it runs through `wally-play` episodes
- Offer a headless no-op alternative for CI / tests / no-display environments
- Allow operators to abort an episode interactively (`q` / `Esc`) without
  leaving the planner loop in a half-finished state

Components:
- `FrameViewer` (in `src/agent/viewer.py`): OpenCV-backed window with optional
  HUD overlay (step counter, plan cost, FPS, done flag) and `q`/`Esc` quit
  detection. `cv2` is imported lazily on first construction.
- `NullViewer`: no-op with the same `show` / `should_quit` / `close` interface
